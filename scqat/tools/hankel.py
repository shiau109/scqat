...
* Seed initial guesses for a curve fit (e.g.
  :class:`scqat.tools.fit_multi_damped_oscillation.FitMultiDampedOscillation`
  or an exponential-decay fit).
* Standalone modal analysis of a ringdown / decay signal.

This lives in ``tools`` (a method, not an experiment) so any fitter,
estimator, or workflow can reuse it without inverting the dependency layering.
"""
