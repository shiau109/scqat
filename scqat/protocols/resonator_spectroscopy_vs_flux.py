"""
Resonator Spectroscopy vs Flux Protocol
=======================================
...
For every flux-bias slice the analyser delegates to
:class:`~scqat.protocols.resonator_spectroscopy.ResonatorSpectroscopyAnalyzer`
...
scope** here — this analyser only produces the trace and the 2-D map for
plotting.
...
class ResonatorSpectroscopyVsFluxAnalyzer(BaseAnalyzer):
    """
    ...
    protocol_name = "resonator_spectroscopy_vs_flux"
    ...
    def _check_data(self, dataset: xr.Dataset) -> None:
        for coord in ("flux_bias", "detuning"):
            if coord not in dataset.coords:
                raise ValueError(
                    f"ResonatorSpectroscopyVsFluxAnalyzer requires a '{coord}' coordinate."
                )
        if "IQdata" not in dataset and not ("I" in dataset and "Q" in dataset):
            raise ValueError(
                "ResonatorSpectroscopyVsFluxAnalyzer requires an 'IQdata' variable, or both 'I' and 'Q'."
            )
    ...
    def extract_parameters(self, dataset: xr.Dataset, **kwargs) -> Dict[str, Any]:
        ...
        Each slice is handed to ``ResonatorSpectroscopyAnalyzer.extract_parameters``
        ...
        n_sigma : float, optional
            ...
            (default 3.0). Not forwarded to the per-slice analyzer.
        ...
        ds = ResonatorSpectroscopyAnalyzer._with_iqdata(dataset)
        ...
        single = ResonatorSpectroscopyAnalyzer()
        n_flux = len(flux)
