"""
Resonator Flux Dispersion Protocol
==================================
...
:class:`~scqat.protocols.resonator_spectroscopy_vs_flux.ResonatorSpectroscopyVsFluxAnalyzer`.
...
class ResonatorFluxDispersionAnalyzer(BaseAnalyzer):
    """Fit ``center_frequency(flux)`` with the full-transmon dispersive model."""

    protocol_name = "resonator_flux_dispersion"
    ...
    def _check_data(self, dataset: xr.Dataset) -> None:
        if "flux_bias" not in dataset.coords:
            raise ValueError("ResonatorFluxDispersionAnalyzer requires a 'flux_bias' coordinate.")
        if "center_freq" not in dataset:
            raise ValueError("ResonatorFluxDispersionAnalyzer requires a 'center_freq' variable.")
