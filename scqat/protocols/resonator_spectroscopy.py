"""
Resonator Spectroscopy Protocol
===============================
Single-dip Lorentzian fitting for resonator spectroscopy data.

The analyser fits the readout **power** ``|IQdata|^2`` (``I^2 + Q^2``): it
...
This mirrors :class:`~scqat.protocols.qubit_spectroscopy.QubitSpectroscopyAnalyzer`
...
class ResonatorSpectroscopyAnalyzer(BaseAnalyzer):
    """
    ...
    protocol_name = "resonator_spectroscopy"
    ...
    def _with_iqdata(dataset: xr.Dataset) -> xr.Dataset:
        ...
        raise ValueError(
            "ResonatorSpectroscopyAnalyzer requires an 'IQdata' variable, or both 'I' and 'Q'."
        )

    def _check_data(self, dataset: xr.Dataset) -> None:
        if "detuning" not in dataset.coords:
            raise ValueError(
                "ResonatorSpectroscopyAnalyzer requires a 'detuning' coordinate."
            )
