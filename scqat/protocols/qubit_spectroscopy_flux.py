"""
Qubit Spectroscopy vs Flux Protocol
===================================
...
:class:`~scqat.protocols.qubit_spectroscopy.QubitSpectroscopyAnalyzer` (peak
...
from scqat.math_tools.robust import mad_outliers


class QubitSpectroscopyFluxAnalyzer(BaseAnalyzer):
    ...
    protocol_name = "qubit_spectroscopy_flux"
    ...
    def _check_data(self, dataset: xr.Dataset) -> None:
        for coord in ("flux_bias", "detuning"):
            if coord not in dataset.coords:
                raise ValueError(
                    f"QubitSpectroscopyFluxAnalyzer requires a '{coord}' coordinate."
                )
    ...
    def extract_parameters(self, dataset: xr.Dataset, **kwargs) -> Dict[str, Any]:
        ...
        Each slice is handed to ``QubitSpectroscopyAnalyzer.extract_parameters``
        ...
        if signal_var is None and "IQdata" not in ds:
            if "I" in ds and "Q" in ds:
                ...
                raise ValueError(
                    "QubitSpectroscopyFluxAnalyzer needs an 'IQdata' variable, both 'I' and "
        ...
        single = QubitSpectroscopyAnalyzer()
